...
To run the simulation, set the desired docker command in the top of `Simulation/simulator.py` file. Then, copy the directory `Simulation/Ocean` with the name `Simulation/OceanDocker` to the docker directory (you can change it in `Config/sim_config.yaml`). Then, use the following code:
